...
const Counter: React.FC<CounterProps & StylesProps> = ({ initialValue = 0, ...props }) => {
  ...
  return (
    <StyledCounter {...props}>
      <Flex border="1px solid" width="fit-content" padding="5px 20px" borderradius="24px" alignitems="center">
      <Button padding="5px 10px" borderradius="24px" margin="0 10px 0 0" onClick={() => dispatch(increment())}>+</Button>
      <Text margin="0 10px 0 0">{counter.data}</Text>
      <Button padding="5px 10px" borderradius="24px" onClick={() => dispatch(decrement())}>-</Button>
      </Flex>
    </StyledCounter>
